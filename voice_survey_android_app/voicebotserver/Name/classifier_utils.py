...
#sys.path.append(os.path.join(root,'libsvm-3.23/python'))
...
def loadModel_2():
    model = pickle.loads(root+'/Name/libsvm.model')
    return model
...
def predictMain_2(model, vocabData, xp, yp = None):
    if yp==None:
        yp = np.array([0]*len(xp))
    dataMade = vectoriseData(xp, vocabData)
    testy = yp.squeeze()
    labels = model.predict(dataMade)
    # labels = svm_predict(testy, dataMade, model, '-q')
    labels_output = np.array(labels[0])
    labels_confidence = np.array(labels[2])
