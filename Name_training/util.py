...
root = os.getcwd()
os.chdir(root+'/libsvm-3.23/python')
sys.path.append(root+'/libsvm-3.23/python')
...
def getfivegrams(t, pn):
    ...
    print("t : ", t)
    print("pn : ", pn)
    print("Type of pn : ", type(pn))
    if not (pn == "no name"):
        
        names = pn.split(" ")[1:-1]
        print("name initial ", names)
        while "'," in names:
            names.remove("',")
        while "'" in names:
            names.remove("'")
...
def predictMain(model, vocabData, xp, yp = None):
    ...
    print("Len of vocab :", len(vocabData))
    dataMade = vectoriseData(xp, vocabData) # One hot encoding
    print("Number of ones : ", sum(dataMade[0]))
    print("Date Made ", dataMade.shape)
    testy = yp.squeeze()
